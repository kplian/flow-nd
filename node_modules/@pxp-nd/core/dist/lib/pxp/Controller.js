...
class Controller {
    constructor(module, Entity = null, config = {
        ...
        this.user = {}; //User;
        this.basicRoutes = [
            { requestMethod: RouteDefinition_1.Method.post, path: '/add', methodName: 'add' },
            { requestMethod: RouteDefinition_1.Method.delete, path: '/delete/:id', methodName: 'delete' },
            { requestMethod: RouteDefinition_1.Method.patch, path: '/edit/:id', methodName: 'edit' },
            { requestMethod: RouteDefinition_1.Method.get, path: '/list', methodName: 'list' }
        ];
        this.basicReadOnly = {
            add: false,
            edit: false,
    ...
    initializeRoutes() {
        ...
        // get type return
        let ishtml = Reflect.getMetadata('ishtml', this.constructor) || {};
        const defaultConfig = Reflect.getMetadata('optionsRoute', this.constructor) || {};
        // define basic routes
        if (this.modelString !== '') {
        ...
        routes.forEach((route) => {
            ...
            // auth
            const auth = authentication[route.methodName] !== undefined ? authentication[route.methodName] : (defaultConfig[route.methodName] ? defaultConfig[route.methodName].authentication : true);
            if (!auth) {
                ...
                [
                    ...
                    this.validated = false;
                    try {
                        await index_1.__(this.genericMethodWrapper(params, req, next, res, route.methodName, methodDbSettings, readOnlyMethod, false, logKey, logValue, isHtmlMethod));
                    }
                    catch (ex) {
                        const now = new Date();
                        ...
                        params, ex.stack, ex.statusCode, endsAt, logValue));
                ...
                [
                    ...
                    this.validated = false;
                    try {
                        await index_1.__(this.genericMethodWrapper(params, req, next, res, route.methodName, methodDbSettings, readOnlyMethod, permission[route.methodName], logKey, logValue, isHtmlMethod));
                    }
                    catch (ex) {
                        const now = new Date();
                        const iniAt = req.start;
    ...
    async genericMethodWrapper(params, req, next, res, methodName, dbsettings, readonly, permission = true, log = true, logConfig = {}, isHtml) {
        if (dbsettings === 'Orm') {
            await index_1.__(this.ormMethodWrapper(params, req, next, res, methodName, readonly, permission, log, logConfig, isHtml));
        }
    ...
    async ormMethodWrapper(params, req, next, res, methodName, readonly, permission = true, log = true, logConfig = {}, isHtml = false) {
        let metResponse;
        if (permission) {
    ...
    getWhereOwnColumns(params, model) {
        const connection = typeorm_1.getConnection(process.env.DB_WRITE_CONNECTION_NAME);
        let ownColumnsForSchema = {};
