import ActionController from './Action';
import ActionTypeController from './ActionType';
import EventController from './Event';
import FieldMapController from './FieldMap';
import FlowController from './Flow';
import FlowInstanceController from './FlowInstance';
import NodeController from './Node';
import NodeInstanceController from './NodeInstance';
import OriginNameController from './OriginName';

export { ActionController, ActionTypeController, EventController, FieldMapController, FlowController, FlowInstanceController, NodeController, NodeInstanceController, OriginNameController };
export const controllers = [ActionController, ActionTypeController, EventController, FieldMapController, FlowController, FlowInstanceController, NodeController, NodeInstanceController, OriginNameController];