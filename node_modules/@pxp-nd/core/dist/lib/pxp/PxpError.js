...
class PxpError extends Error {
    constructor(statusCode, message, errorObject = undefined, stack) {
        super();
        this.statusCode = statusCode;
        this.message = statusCode === 500 ? genericMessage : message;
        this.errorObject = errorObject;
        if (errorObject && errorObject.response && errorObject.config) {
            this.tecMessage = `Error in Axios call to ${this.errorObject.config.url}, response: ${JSON.stringify(this.errorObject.response.data)}`;
        }
        else {
...
const __ = (promise, myShowError = false) => promise
    ...
    if (error instanceof PxpError) {
        ...
        throw new PxpError(showError ? 406 : 500, error.message, error, error.stack);
    }
});
exports.__ = __;
...
const errorMiddleware = (err, req, res) => {
    ...
    const extraObj = process.env.NODE_ENV === "production"
        ? {}
